        ...
        <div class="bg-white shadow-2xl rounded-3xl overflow-hidden flex min-h-[600px] border border-gray-100">
            ...
            <div class="w-3/5 p-8 border-r border-gray-100 flex flex-col">
                ...
                <div class="flex items-center justify-between mb-8 flex-shrink-0">
                    <div class="flex items-center space-x-2">
                        <button wire:click="previousMonth" wire:loading.attr="disabled" class="p-2 text-orange-400 hover:text-orange-600 hover:bg-orange-50 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <button wire:click="nextMonth" wire:loading.attr="disabled" class="p-2 text-orange-400 hover:text-orange-600 hover:bg-orange-50 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- 日付グリッド --}}
                <div class="grid grid-cols-7 gap-3 flex-grow transition-opacity duration-300 mb-6" wire:loading.class="opacity-40">
                    @php
            ...
            <div class="mt-auto bg-gray-50 p-4 rounded-2xl border border-gray-100 flex items-center shadow-inner h-40">
                <div class="w-1/3 pr-4 border-r border-gray-200">
                    <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Monthly Balance</h3>
                    <p class="text-sm font-bold text-gray-700 leading-tight">今月の<br>振り返り傾向</p>
                </div>
        ...
        <div class="w-2/5 p-8 bg-gradient-to-br from-orange-50 to-white flex flex-col justify-between max-h-[700px]">
            <div class="flex flex-col h-full overflow-hidden">
                ...
                <div class="p-6 bg-white rounded-3xl shadow-sm border border-orange-300 ring-4 ring-orange-50 flex-shrink-0">
                    ...
                    {{-- 🚀 【追加】前回書いた「明日へのアクション」がある場合のみ表示 --}}
                    @if(count($previousActions) > 0)
                    <div class="mt-3 p-3 bg-blue-50/50 border border-blue-100 rounded-xl">
                        <p class="text-[10px] font-bold text-blue-500 mb-1 flex items-center">
                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                            </svg>
                            前回（{{ $previousActionDate }}）の「明日へのアクション」
                        </p>
                        <ul class="space-y-1">
                            @foreach($previousActions as $action)
                            <li class="text-xs text-gray-600 font-medium flex items-start pl-1">
                                <span class="text-blue-400 mr-1.5 flex-shrink-0">・</span>
                                <span class="break-all">{{ $action->content }}</span>
                            </li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <div class="mt-4 flex-shrink-0 pt-2 border-t border-orange-100">
                    <a href="{{ route('reflection', ['date' => $selectedDate]) }}" wire:navigate class="block text-center w-full py-4 bg-pink-400 hover:bg-pink-500 text-white text-lg font-black rounded-3xl shadow-lg shadow-pink-100 transition-all transform hover:-translate-y-1 active:scale-95">
                        キャンバスを開く
                    </a>
                </div>
                @endif
